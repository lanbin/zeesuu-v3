const setViewport = function (createViewportMeta = { width: 0, debug: false }) {
    if (!window || !document) {
        return;
    }
    ...
    if (metaEl && !matchScale && matchWidth && matchWidth[1] != 'device-width') {
        var width = parseInt(matchWidth[1]), iw = window.innerWidth || width, ih = window.innerHeight ||
            window.outerHeight ||
            window.screen.height ||
            window.screen.availHeight, ow = window.outerWidth || iw, sw = window.screen.width || iw, saw = window.screen.availWidth || iw, ish = window.screen.height || ih, sah = window.screen.availHeight || ih, w = Math.min(iw, ow, sw, saw, ish, sah), scale = w / width;
        if (scale < 1) {
